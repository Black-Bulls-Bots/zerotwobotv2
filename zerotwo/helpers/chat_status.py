...
def check_admin(
    ...
    def wrapper(func):
        ...
        async def wrapped(
            ...
            if chat.type == ChatType.PRIVATE and not (only_owner):
            ...
            if only_owner:
                if isinstance(user_member, ChatMemberOwner) or user.id == OWNER_ID:
                    return await func(update, context, *args, **kwargs)
                else:
            ...
            if permission:
                ...
                if is_user:
                    if isinstance(user_member, ChatMemberOwner):
                        ...
                        else False or user.id == OWNER_ID
                    ):
                        return await func(update, context, *args, **kwargs)
                    elif no_reply:
                        return
...
from zerotwo.modules import connection
