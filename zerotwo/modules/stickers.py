...
from telegram import InlineKeyboardButton, InlineKeyboardMarkup, Update, User, Message, InputSticker
...
from zerotwo.helpers.misc import convert_gif
...
async def cb_sticker(update: Update, context: ContextTypes.DEFAULT_TYPE):
    #TODO: fix searching stickers, combot.org moved to tailwindcss and deployed cloudflare
    msg = update.effective_message
    split = msg.text.split(" ", 1)
    headers = {"User-Agent": "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/137.0.0.0 Safari/537.36", "accept": "*/*"}
    if len(split) == 1:
        await msg.reply_text("Provide some name to search for pack.")
        return

    # async with AsyncClient() as client:
    #     r = await client.get(combot_stickers_url + split[1], headers=headers)

    scraper = cloudscraper.create_scraper()

    r = scraper.get(combot_stickers_url + split[1])
    text = r.text
    soup = bs(text, "lxml")
    results = soup.find_all("a", {"class": "hover:tw-bg-[#3390ed]/90"})
    titles = soup.find_all("class", "stickerset__title")
    if not results:
...
async def kang(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if msg.reply_to_message and not msg.reply_to_message.forum_topic_created:
        ...
        if not is_animated and not (is_video or is_gif):
            try:
                ...
                await context.bot.add_sticker_to_set(
                    user_id=user.id,
                    name=packname,
                    sticker=InputSticker(open(f"kangsticker_{user.id}.png", "rb"), sticker_emoji, format=StickerFormat.STATIC)
                )
            ...
            try:
                await context.bot.add_sticker_to_set(
                    user_id=user.id,
                    name=packname,
                    sticker=InputSticker(open(f"kangsticker_{user.id}.tgs", "rb"), sticker_emoji, format=StickerFormat.ANIMATED)
                )
                await msg.reply_text(
                    f"Sticker successfully added to [pack](t.me/addstickers/{packname})"
                    + f"\nEmoji is: {sticker_emoji}",
            ...
            try:
                await context.bot.add_sticker_to_set(
                    user_id=user.id,
                    name=packname,
                    sticker=InputSticker(open(f"kangsticker_{user.id}.webm", "rb"), sticker_emoji, format=StickerFormat.VIDEO)
                )
                await msg.reply_text(
                    f"Sticker Successfully added to [pack](t.me/addstickers/{packname})"
                    + f"\nEmoji is: {sticker_emoji}",
        ...
        try:
            ...
            await msg.reply_text("I can only kang images!.")
            LOGGER.error(e)
            return
        except TelegramError as e:
            if e.message == "Stickerset_invalid":
    ...
    try:
        ...
        if png_sticker:
            success = await context.bot.create_new_sticker_set(
                ...
                stickers=[InputSticker(png_sticker, emoji, StickerFormat.STATIC)],
            )
        if tgs_sticker:
            success = await context.bot.create_new_sticker_set(
                user.id,
                ...
                stickers=[InputSticker(tgs_sticker, emoji, StickerFormat.ANIMATED)],
            )
        if webm_sticker:
            success = await context.bot.create_new_sticker_set(
                user.id,
                ...
                stickers=[InputSticker(webm_sticker, emoji, StickerFormat.VIDEO)],
            )
